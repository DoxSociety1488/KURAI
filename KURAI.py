...
'''
    ...
    txt = Fore.WHITE + '[KURAI INFO] Данная программа была создана пользователем @GD23C, все вопросы к @GD23C перепродажа запрещенна'
    for i in txt:
        time.sleep(0.04)
...
''')
    ...
    if Choose == '2':
        ...
        Choosing()
    if Choose == '3':
        txt = Fore.WHITE + '[W.I.P] Данная функция временно не работает'
        for p in txt:
            time.sleep(0.04)
            print(p, end='', flush=True)
        time.sleep(10)
...
def web_crawler(url, depth=2):
    ...
    while queue:
        ...
        print("KURAI CRAWLING:", current_url)
